/*
Modification of pbp_django_auth 0.4.0 by Tim Asdos PBP
...
- Added CookieRequest.postFormData() to allow sending files to server from client
 */
...
class CookieRequest {
  ...
  Future<dynamic> postFormData(String url, dynamic data, dynamic files) async {
    await init();
    if (kIsWeb) {
      dynamic c = _client;
      c.withCredentials = true;
  ...
  Future _updateCookie(http.BaseResponse response) async {
    await init();
